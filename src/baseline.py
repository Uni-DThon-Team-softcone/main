...
class CFG:
    ...
    JSON_DIR: str = "./data/train/press_json"
    JPG_DIR: str = "./data/train/press_jpg"
...
class UniDSet(Dataset):
    def __init__(self, json_files: List[str], jpg_dir: str = None, vocab: Vocab = None,
        ...
        for jf in json_files:
            # 1) JSON 읽기 (비어있거나 깨진 파일은 스킵)
            try:
                data = read_json(jf)
            except json.JSONDecodeError as e:
                print(f"[WARN] Skip (JSON decode error): {jf} ({e})")
                continue
            except Exception as e:
                print(f"[WARN] Skip (read failed): {jf} ({e})")
                continue

            # 2) 시각 질의(annotation) 선별. 없으면 이미지 해석도 불필요하므로 스킵
            anns = data.get("learning_data_info", {}).get("annotation", [])
            valid_anns = [a for a in anns if is_visual_ann(a)]
            if not valid_anns:
                continue

            # 3) 이미지 경로 확인 실패 시 해당 파일 스킵
            try:
                img_path = get_image_path(jf, data, jpg_dir=jpg_dir)
            except FileNotFoundError as e:
                print(f"[WARN] Skip (image not found): {jf} ({e})")
                continue
